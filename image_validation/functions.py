...
def tif_compression(file_id, filename, db_cursor, logger):
    ...
    (out, err) = p.communicate()
    logger.debug("tif_compression_out: {} {}".format(file_id, out.decode('UTF-8')))
    logger.debug("tif_compression_err: {} {}".format(file_id, err.decode('UTF-8')))
...
def file_exif(file_id, filename, filetype, db_cursor, logger):
    ...
    for line in exif_info.splitlines():
        ...
        try:
            ...
            logger.error("exif: {} {}".format(file_id, e))
            continue
    return True
...
def file_pair_check(file_id, filename, derivative_path, derivative_type, db_cursor):
    ...
    if len(derivative_file) == 0:
        ...
        # Raw file is missing
        file_pair = 1
        file_pair_info = "Missing related {} file".format(derivative_type)
    else:
...
def update_folder_stats(folder_id, folder_path, db_cursor, logger):
    """
