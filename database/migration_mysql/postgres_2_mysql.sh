...
psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select tableID as table_id, folder_id, md5_type, md5, updated_at::timestamp from folders_md5) to 'folders_md5.csv' CSV HEADER;"
...
psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select file_id, folder_id, file_name, file_timestamp::timestamp, dams_uan, preview_image, updated_at::timestamp, created_at::timestamp from files) to 'files.csv' CSV HEADER;"


psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select file_id, folder_id, file_check, check_results, check_info, updated_at::timestamp from file_checks) to 'files_checks.csv' CSV HEADER;"


psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select file_id, filetype, md5, updated_at::timestamp from file_md5) to 'file_md5.csv' CSV HEADER;"



psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select file_id, filetype, filesize, updated_at::timestamp from files_size) to 'files_size.csv' CSV HEADER;"


psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select tableid as table_id, file_id, link_name, link_url, link_notes, updated_at::timestamp from files_links) to 'files_links.csv' CSV HEADER;"
...
psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select file_id, post_step, post_results, post_info, updated_at::timestamp from file_postprocessing) to 'file_postprocessing.csv' CSV HEADER;"
...
psql -U osprey -h si-fsosprey01.si.edu osprey -c "\copy (select user_id, username, full_name, pass, CASE WHEN user_active is True THEN 1 ElSE 0 END as user_active, CASE WHEN is_admin is True THEN 1 ElSE 0 END as is_admin from qc_users) to 'qc_users.csv' CSV HEADER;"
