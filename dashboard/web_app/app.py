...
def user_perms(project_id, user_type='user'):
    ...
    if user_type == 'user':
        query = sql.SQL(
            "SELECT COUNT(*) as is_user FROM qc_projects p, users u WHERE p.user_id = u.user_id AND p.project_id = %(project_id)s AND u.username = %(user_name)s")
...
def login():
    ...
    print(query_database(
        "SELECT SUM(objects_digitized) as total from projects_stats where project_id NOT IN (SELECT project_id FROM projects WHERE skip_project IS True)"))

    summary_stats['objects_digitized'] = "{:,}".format(query_database(
        "SELECT SUM(objects_digitized) as total from projects_stats where project_id NOT IN (SELECT project_id FROM projects WHERE skip_project IS True)")[
                                                           0]['total'])
    summary_stats['images_captured'] = "{:,}".format(query_database(
        "SELECT SUM(images_taken) as total from projects_stats where project_id NOT IN (SELECT project_id FROM projects WHERE skip_project IS True)")[
                                                         0]['total'])
    summary_stats['digitization_projects'] = "{:,}".format(
        query_database("SELECT COUNT(*) as total FROM projects WHERE skip_project IS NOT True")[0]['total'])
    summary_stats['active_projects'] = "{:,}".format(query_database(
        "SELECT COUNT(*) as total FROM projects WHERE skip_project IS NOT True AND project_status='Ongoing'")[0][
                                                         'total'])
    summary_stats['images_public'] = "{:,}".format(query_database(
        "SELECT SUM(images_public) as total FROM projects_stats WHERE project_id NOT IN (SELECT project_id FROM projects WHERE skip_project IS True)")[
                                                       0]['total'])
    ...
    md_stats['images_public'] = "{:,}".format(query_database(
        "SELECT SUM(images_public) as total FROM projects_stats WHERE project_id IN (SELECT project_id FROM projects WHERE skip_project IS NOT True AND project_section = 'MD')")[
                                                  0]['total'])
...
def dashboard_f(project_alias=None, folder_id=None):
    ...
    folder_check = query_database(
        "SELECT folder_id FROM folders WHERE folder_id = %(folder_id)s AND project_id IN (SELECT project_id FROM projects WHERE project_alias = %(project_alias)s)",
    ...
    project_info = query_database("SELECT * FROM projects WHERE project_alias = %(project_alias)s",
                                  {'project_alias': project_alias})[0]
    filechecks_list_temp = query_database(
        "SELECT settings_value as file_check FROM projects_settings WHERE project_setting = 'project_checks'  and project_id = %(project_id)s",
    ...
    if folder_id is not None and folder_id != '':
        ...
        folder_files_df = pd.DataFrame(
            query_database("SELECT file_id, file_name FROM files WHERE folder_id = %(folder_id)s",
                           {'folder_id': folder_id}))
        no_items = 25
        if page == 1:
            offset = 0
    ...
    # Reports
    reports = query_database("SELECT * FROM data_reports WHERE project_id = %(project_id)s ORDER BY report_title_brief",
                             {'project_id': project_id})
...
def dashboard(project_alias=None):
    ...
    # Reports
    reports = query_database("SELECT * FROM data_reports WHERE project_id = %(project_id)s ORDER BY report_title_brief", {'project_id': project_id})
...
def file(file_id=None):
    ...
    project_alias = \
    query_database("SELECT COALESCE(project_alias, CAST(project_id AS char)) as project_id FROM projects "
                   " WHERE project_id = %(project_id)s",
    ...
    file_metadata = pd.DataFrame(query_database("SELECT tag, taggroup, tagid, value "
                                                ...
                                                " ORDER BY taggroup, tag ",
                                                {'file_id': file_id}))
    file_links = query_database("SELECT link_name, link_url FROM files_links WHERE file_id = %(file_id)s ",
                                {'file_id': file_id})
    if current_user.is_authenticated:
        user_name = current_user.name
        is_admin = user_perms('', user_type='admin')
            ...
            if query_type is not None and query_property is not None and query_value is not None:
                ...
                if query_type == "folder":
                    ...
                    if folder_id is not None:
                        if query_property == "status0":
                            ...
                            query = sql.SQL(
                                "UPDATE folders f SET no_files = d.no_files FROM (SELECT COUNT(DISTINCT f.file_id) AS no_files FROM files_checks c, files f WHERE f.folder_id = %s AND f.file_id = c.file_id AND c.check_results = 1) d WHERE f.folder_id = d.folder_id")
                            res = query_database_insert(query, (folder_id,))
                            query = sql.SQL(
                                "UPDATE folders f SET file_errors = CASE WHEN d.no_files > 0 THEN 1 ELSE 0 END FROM (SELECT COUNT(DISTINCT f.file_id) AS no_files FROM files_checks c, files f WHERE f.folder_id = %s AND f.file_id = c.file_id AND c.check_results = 9) d WHERE f.folder_id = d.folder_id")
                            res = query_database_insert(query, (folder_id,))
                            query = sql.SQL(
                                "UPDATE folders f SET no_files = d.no_files FROM (SELECT count(*) AS no_files, folder_id FROM files WHERE folder_id = %s GROUP BY folder_id) d WHERE f.folder_id = d.folder_id")
                            res = query_database_insert(query, (folder_id,))
                        elif query_property == "md50":
                            query = sql.SQL(
                                "INSERT INTO folders_md5 (folder_id, md5_type, md5) VALUES (%s, %s, 0) ON CONFLICT (folder_id, md5_type) DO UPDATE SET md5 = 0")
                            res = query_database_insert(query, (folder_id, query_value))
                        elif query_property == "md51":
                            query = sql.SQL(
                                "INSERT INTO folders_md5 (folder_id, md5_type, md5) VALUES (%s, %s, 1) ON CONFLICT (folder_id, md5_type) DO UPDATE SET md5 = 1")
                            res = query_database_insert(query, (folder_id, query_value))
                ...
                if query_type == "file":
                    ...
                    if query_property == "filechecks":
                        ...
                        query = sql.SQL(
                            "INSERT INTO files_checks (file_id, folder_id, file_check, check_results, check_info, updated_at) VALUES (%s, %s, %s, %s, %s, NOW()) ON CONFLICT (file_id, file_check) DO UPDATE SET check_results = %s, check_info = %s, updated_at = NOW()")
                        logging.info(query)
                        res = query_database_insert(query, (
                        file_id, folder_id, file_check, check_results, check_info, check_results, check_info))
                        logging.info(res)
                    elif query_property == "filemd5":
                        filetype = request.form.get("filetype")
                        folder_id = request.form.get("folder_id")
                        query = sql.SQL(
                            "INSERT INTO file_md5 (file_id, filetype, md5) VALUES (%s, %s, %s) ON CONFLICT (file_id, filetype) DO UPDATE SET md5 = %s")
                        ...
                        res = query_database_insert(query, (
                        file_id, folder_id, check_results, check_info, check_results, check_info))
                        ...
                        for line in query_value.splitlines():
                            ...
                            try:
                                tag = re.split(r'\t+', line)
                                query = sql.SQL(
                                    "INSERT INTO files_exif (file_id, filetype, tagid, taggroup, tag, value) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (file_id, filetype, tag) DO UPDATE SET value = %s")
                                logging.info(query)
                                res = query_database_insert(query,
                                                            (file_id, filetype, tag[1], tag[0], tag[3], tag[2], tag[2]))
                                logging.info(res)
            ...
            if query_type is not None:
                if query_type == "folder":
                    ...
                    if folder is not None and folder_path is not None:
                        query = sql.SQL(
                            "INSERT INTO folders (project_folder, path, status, project_id, date) VALUES (%s, %s, 0, %s, %s) RETURNING folder_id")
                        logging.info("2673:{}".format(query))
                        data = query_database_insert(query, (folder, folder_path, project_id, folder_date),
                                                     return_res=True)
                    ...
                    if filename is not None and timestamp is not None and folder_id is not None:
                        query = sql.SQL(
                            "INSERT INTO files (folder_id, file_name, file_timestamp) VALUES (%s, %s, %s) RETURNING file_id")
                        ...
                        query = sql.SQL(
                            "SELECT f.file_id, fol.project_folder FROM files f, folders fol WHERE f.folder_id = fol.folder_id AND f.file_name = %s AND f.folder_id != %s"
                    ...
                    if file_id is not None and filetype is not None and filesize is not None:
                        query = sql.SQL(
                            "INSERT INTO files_size (file_id, filetype, filesize) VALUES (%s, %s, %s) ON CONFLICT (file_id, filetype) DO UPDATE SET filesize = %s")
...
def api_get_report(report_id=None):
    ...
    if report_id is None:
        ...
        query = query_database("SELECT * FROM data_reports WHERE report_id = %(report_id)s",
                               {'report_id': report_id})
        if len(query) == 0:
            query = query_database("SELECT * FROM data_reports WHERE report_alias = %(report_id)s",
                                   {'report_id': report_id})
            if len(query) == 0:
                return None
        if file_name is not None:
            data = query_database("SELECT * FROM ({}) a WHERE file_name = %(file_name)s".format(query[0]['query_api'].replace('%', '%%')), {'file_name': file_name})
        elif dams_uan is not None:
            data = query_database("SELECT * FROM ({}) a WHERE dams_uan = %(dams_uan)s".format(query[0]['query_api'].replace('%', '%%')), {'dams_uan': dams_uan})
        else:
...
def data_reports(project_alias=None, report_id=None):
    ...
    project_id = query_database("SELECT project_id FROM projects WHERE project_alias = %(project_alias)s",
                                {'project_alias': project_alias})
    ...
    project_report = query_database("SELECT * FROM data_reports WHERE "
    ...
    report_data = pd.DataFrame(query_database(project_report[0]['query']))

    report_data_updated = query_database(project_report[0]['query_updated'])[0]['updated_at']
